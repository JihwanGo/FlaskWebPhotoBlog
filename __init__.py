...
def print_settings(config):
    print('SETTINGS for PHTOLOG APPLICATION')
...
def create_app(config_filepath='resource/config.cfg'):
    ...
    # 기본 설정 PhotologConfig 객체에 정의 운영 환경 또는 기본 설정을 변경 하려면
    # 실행 환경변수인 Phtolog_settings에 변경할 설정을 담고 있는 파일 경로를 설정
    from photolog.photolog_config import PhotologConfig
    photolog_app.config.form_object(PhotologConfig)
    ...
    # 뷰 함수가 잇는 모듈을 임포트해야 해당 뷰 함수들을 인식할 수 있음
    ...
    # SessionInterface 설정.
    # Redis를 이용한 세션 cache_session.RedisCacheSessionInterface 임포트하고
    # app.session_interface에 RedisCacheSessionInterfce를 할당
    from photolog.cache_session import SimpleCacheSessionInterface
    photolog_app.session_interface = SimpleCacheSessionInterface()

    # 공통으로 적용할 HTTP 404와 500 에러 핸들러 설정

    photolog_app.error_handler_spec[None][404] = not_found
    photolog_app.error_handler_spec[None][500] = sserver_error
